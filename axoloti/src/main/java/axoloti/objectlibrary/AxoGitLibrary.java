...
public class AxoGitLibrary extends AxolotiLibrary {
    ...
    public void sync() {
        ...
        if (git != null) {
            ...
            if (isDirty) {
                Logger.getLogger(AxoGitLibrary.class.getName()).log(Level.INFO, "Library modifications detected: {0}", logDetails());
            }
            if (isDirty && isAuth()) {
                if (!add(git)) {
                    git.getRepository().close();
                ...
                Logger.getLogger(AxoGitLibrary.class.getName()).log(Level.INFO, "Library modifications uploaded: {0}", logDetails());
                reportStatus(git);
            }
            ...
            Logger.getLogger(AxoGitLibrary.class.getName()).log(Level.INFO, "Library sync successful: {0}", logDetails());
            git.getRepository().close();
        } else {
            Logger.getLogger(AxoGitLibrary.class.getName()).log(Level.WARNING, "Library sync failed. Cannot find submodule: {0}", logDetails());
        }
    }
    ...
    public void init(boolean delete) {
        ...
        if (getRemoteLocation() == null || getRemoteLocation().length() == 0) {
            Logger.getLogger(AxoGitLibrary.class.getName()).log(Level.WARNING, "Library init failed. No remote specified: {0}", logDetails());
            return;
        }

        if(ldir.exists() && !delete) {
            return;
        }
        ...
        try {
            Git git = cmd.call();
            git.getRepository().close();
            Logger.getLogger(AxoGitLibrary.class.getName()).log(Level.INFO, "Library repository initialized successfully : {0}", logDetails());
        } catch (Exception ex) {
    ...
    private Git getGit() {
        Git git = null;
        try {
            git = new Git(Git.open(new File(getLocalLocation())).getRepository());
    ...
    private boolean push(Git git) {
        ...
        try {
            ...
            Logger.getLogger(AxoGitLibrary.class.getName()).log(Level.WARNING, "Library sync (push) failed : {0}", logDetails());
            Logger.getLogger(AxoGitLibrary.class.getName()).log(Level.SEVERE, null, ex);
        }
        return false;
    }
