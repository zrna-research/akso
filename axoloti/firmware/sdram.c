...
static const SDRAMConfig sdram_cfg = {
  ...
  .sdtr = (uint32_t)((2   - 1) |  // FMC_LoadToActiveDelay = 2 (TMRD: 2 Clock cycles) [12 - 14 ns]
                     (7 <<  4) |  // FMC_ExitSelfRefreshDelay = 7 (TXSR: min=70ns (7x11.11ns)) [60 - 64 ns]
                     (4 <<  8) |  // FMC_SelfRefreshTime = 4 (TRAS: min=42ns (4x11.11ns) max=120k (ns)) ok
                     (7 << 12) |  // FMC_RowCycleDelay = 7 (TRC:  min=70 (7x11.11ns)) [60 - 64 ns]
                     (2 << 16) |  // FMC_WriteRecoveryTime = 2 (TWR:  min=1+ 7ns (1+1x11.11ns)) [12 - 14ns]
                     (2 << 20) |  // FMC_RPDelay = 2 (TRP:  20ns => 2x11.11ns) ok
                     (2 << 24)),  // FMC_RCDDelay = 2 (TRCD: 20ns => 2x11.11ns) ok
  ...
  //.sdrtr = (1386 << 1),
    ...
    /* if (STM32_SYSCLK == 480000000) ->
     64ms / 8192 = 7.8125us
     7.8125us * 120MHz = 918 - 20 = 898 */
  //.sdrtr = (1386 << 1),
