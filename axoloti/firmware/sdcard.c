...
/* FS object.*/
FATFS SDC_FS SDMMC_MEM_FW;

/* FS mounted and ready.*/
bool fs_ready SDMMC_MEM_FW = FALSE;
BYTE work[FF_MAX_SS] SDMMC_MEM_FW;
uint32_t fbuff[512] SDMMC_MEM_FW;
uint8_t wtf[512] SDMMC_MEM_FW;
/*
...
uint8_t patch_header[sizeof(bin_header_t) + MAX_SEGMENTS*sizeof(bin_segment_t)] SDMMC_MEM_FW;
...
int bin_loader_flash(const void * startloc, uint32_t size) {
...
#if BOARD_AKSO
    // adjust these lengths
    memset((char *) 0x24000000, 0x66, 64 * 1024); // sram1
	memset((char *) 0x24060000, 0x66, 64 * 1024); // sram3
	...
	for (i = 0; i < pre->nsegments; i++) {
		...
		if (!((segment_dest_addr == (void *) 0x0)
              || (segment_dest_addr == (void *) 0x24000000)
              || (segment_dest_addr == (void *) 0x24060000)
...
static int bin_loader_f(FIL *FileObject) {
	...
	err = f_read(FileObject, patch_header, sizeof(fbuff), (UINT*) &bytes_read);
	...
	for (i = 0; i < pre->nsegments; i++) {
        ...
        err = f_read(FileObject, segment_dest_addr, segment_length, (UINT*) &bytes_read);
        if(err != FR_OK) {
            chSysHalt("LDRFILE");
        }
