...
static void ADAU_I2C_Init(void)
{
    if (HAL_I2C_GetState(&ADAU1961_i2c_handle) == HAL_I2C_STATE_RESET)
    {
        ...
        ADAU1961_i2c_handle.Init.DualAddressMode = I2C_DUALADDRESS_DISABLE;
        ADAU1961_i2c_handle.Init.OwnAddress2 = 0;
        ADAU1961_i2c_handle.Init.OwnAddress2Masks = I2C_OA2_NOMASK;
        ADAU1961_i2c_handle.Init.GeneralCallMode = I2C_GENERALCALL_DISABLE;
        ADAU1961_i2c_handle.Init.NoStretchMode = I2C_NOSTRETCH_DISABLE;
...
// SINGLE ENDED (unipolar cv): ADAU1961_WriteRegister(ADAU1961_REG_R4_RMIXL0, 0b1010001); // mixer1 enable, mute LINP and LINR
    // SINGLE ENDED (unipolar cv):    ADAU1961_WriteRegister(ADAU1961_REG_R6_RMIXR0, 0b1010001); // mixer2 enable, mute LINP and LINR
    ...
    ADAU1961_WriteRegister(ADAU1961_REG_R33_PMONO, 0x02);   //MONOM+MOMODE
    ...
    ADAU1961_WriteRegister(ADAU1961_REG_R30_PHPRVOL, 0xC2);

    chThdSleepMilliseconds(10);
}

#if DEBUG_INT_ON_GPIO
...
// SAI1_B is synchronous slave receiver
    ...
    if  (isMaster) {
        sai_a_dma = dmaStreamAlloc(STM32_SAI_A_DMA_STREAM, STM32_SAI_A_IRQ_PRIORITY,
...
#if SPILINK_ENABLED
        b = dmaStreamAlloc(sai_a_dma, STM32_SAI_A_IRQ_PRIORITY,
...
#if !DEBUG_INT_ON_GPIO
    sai_b_dma = dmaStreamAlloc(STM32_SAI_B_DMA_STREAM, STM32_SAI_B_IRQ_PRIORITY,
                               (stm32_dmaisr_t)0,
                               (void *)0);
    dmaSetRequestSource(sai_b_dma, STM32_DMAMUX1_SAI1_B);
#else
    b |= dmaStreamAlloc(sai_b_dma, STM32_SAI_B_IRQ_PRIORITY,
                        dma_sai_b_interrupt,
                        (void *)0);
#endif
    if (sai_b_dma == NULL) {
