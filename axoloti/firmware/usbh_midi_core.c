...
USBH_StatusTypeDef USBH_MIDI_ProcessInput(USBH_HandleTypeDef *phost) {
    ...
    switch (MIDI_Handle->state_in) {
        case MIDI_INIT:
            if ( USBH_Get_StringDesc(
                     phost,
                     phost->device.DevDesc.iProduct,
                     (uint8_t*) MIDI_Handle->name,
                     0xff) == USBH_OK) {
        ...
        case MIDI_POLL:
            if (URB_state_in == USBH_URB_DONE) {
                ...
                USBHMIDIC[0].config.cb_report(&USBHMIDIC[0].config, (uint32_t *)MIDI_Handle->buff_in, n);
                MIDI_Handle->state_in = MIDI_GET_DATA;
            } else if (
                URB_state_in == USBH_URB_STALL ||
                URB_state_in == USBH_URB_ERROR) {
...
#else
        (void)osMessageQueuePutI(phost->os_event, &phost->os_msg, 0U, NULL);
#endif
#endif
    }
    return USBH_OK;
